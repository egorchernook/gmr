...
with os.scandir(".") as it:
    for entry in it:
        if entry.is_file() and entry.name.find("GMR_tw=") != -1:
            ...
            file.close()
            ax.errorbar(x_ax, gmr_lhc, yerr=gmr_lhc_err,
                        label=r'$t - t_w$ = ' + tw, errorevery=100, elinewidth=0.2, capsize=3, capthick=0.5)
...
with os.scandir(".") as it:
    for entry in it:
        if entry.is_file() and entry.name.find("GMR_tw=") != -1:
            ...
            file.close()
            ax.errorbar(x_ax, gmr_uhc, yerr=gmr_uhc_err,
                        label=r'$t - t_w$ = ' + tw, errorevery=100, elinewidth=0.2, capsize=3, capthick=0.5)

...
namespace task
{
    struct sample_t
    {
        using lattice_t = qss::multilayer_system<
            qss::multilayer<
                typename base_config::lattice_t>>;
        using n_lattice_t = qss::multilayer<
            typename base_config::electron_dencity_t>;

        lattice_t lattice;
        n_lattice_t n_up;
        n_lattice_t n_down;

        qss::spin_transport::nanostructure_type<
            qss::lattices::three_d::fcc,
            typename qss::spin_transport::proxy_spin>
            proxy_lattice;

        const config_t config;
        const typename decltype(std::function{base_config::createHamilton_f})::result_type
            hamilt;

        sample_t(lattice_t &&lattice_,
                 n_lattice_t &&n_up_,
                 n_lattice_t &&n_down_,
            ...
            : lattice{lattice_}, n_up{n_up_}, n_down{n_down_},
              ...
              config{config_},
              hamilt{base_config::createHamilton_f(
                  config.field,
                  base_config::getDelta(config.N))} {}

        std::pair<typename base_config::spin_t::magn_t,
                  typename base_config::spin_t::magn_t>
        makeMonteCarloStep()
        {
        ...
        startObservation()
        {
            ...
            const auto [j_up, j_down] =
                qss::algorithms::spin_transport::perform(proxy_lattice);
            constexpr auto area =
                base_config::L * base_config::L;
    ...
    {
        const typename base_config::sizes_t sizes{
            base_config::L,
            base_config::L,
            config.N};
        ...
        const typename base_config::electron_dencity_t
            n_film{n_0, sizes};

        return sample_t{
            typename sample_t::lattice_t{qss::multilayer<
                typename base_config::lattice_t>{{fst_film, snd_film}, {base_config::J2}}},
    ...
    void prepare(sample_t &sam)
    {
        ...
        auto hamilt =
            base_config::createHamilton_f(config.field,
                                          Delta);

...
struct base_config {
    ...
    constexpr static std::array<double, 10> deltas{
        0.5, 0.6, 0.636, 0.7, 0.734, 0.77, 0.816, 0.85, 0.882, 0.9};
    ...
    constexpr static std::array t_wait_vec{
        0u}; //, 100U, 200u, 400u, 1000u}; // должен быть отсортирован по увеличению
    ...
    constexpr static double A_fb = -0.8;
    ...
    static std::vector<config_t> getConfigs()
    {
        ...
        result.reserve(
            m_stat_amount * N_size_vec.size() * T_creation_vec.size() * T_sample_vec.size()
            * magn_field_vec.size());
